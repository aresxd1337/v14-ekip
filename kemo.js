...
(async () => {
	try {
		console.log('[KEMO] Komutlar yükleniyor.');
    ...
    await rest.put(
      ...
      { body: commands },
		...
		console.log('[KEMO] Komutlar yüklendi.');
	} catch (error) {
		console.error(error);
	}
})();
...
client.on('modalSubmit', async (modal) => {
  ...
  var LogChannel = client.guilds.cache.get(ayar.GuildID).channels.cache.find((channel) => channel.id === ayar.BaşvuruLogChannelID);
    if(modal.customId === 'ybasvuru') {
      ...
      if (hakkında) {
  let kemo = new Discord.MessageEmbed()
  .setDescription(`
  **${modal.user.tag}** - (\`${modal.user.id}\`) **Kullanıcısının Başvuru Formu**
  ...
  .setTimestamp()
        ...
        await LogChannel.send({ content: `<@&${ayar.YetkiliAlımRoleID}> ${modal.user}`,  embeds: [kemo] })      
      }
    }  
  });
